...
const FilterSection = ({ onClose, onApply, filters, setFilters }) => {
    ...
    return (
        ...
        >
            ...
            {/* Tour Types */}
            <View style={filterStyles.section}>
                <Text style={filterStyles.sectionTitle}>Tour Type</Text>
                {tourTypes.map((type) => (
                    <TouchableOpacity
                        key={type}
...
const TourPackageSearchResults = ({ route, navigation }) => {
    const dispatch = useDispatch();
    const { myData } = route.params;
    useEffect(() => {
        const job = async () => {
            await SecureStore.setItemAsync("tourSearchroute", JSON.stringify(myData));
        }
        job()
    }, [myData])
    ...
    useEffect(() => {
        ...
        const job = () => {
            ...
            if (filters.duration !== "" || appfilters.length) {
                dispatch(getTourPackages({ area: myData.location, month: month, tourTypes: filters.selectedCarTypes, duration: filters.duration }))
            } else {
                dispatch(getTourPackages({ area: myData.location, month: month }))
            }
        }
        job()
    }, [myData, applyFilter])
    ...
    const handleSearch = () => {
        ...
        const date = formatToDDMMYYYY(tourDate)
        const data = {
            location: myData.location,
            travelDate: date
        }
        // console.log(data)
        navigation.navigate("toursearch", { myData: data });
    }
...
export default TourPackageSearchResults;
